...
namespace gx {
    ...
    void AssimpLoader::processNode(
        ...
        if (!scene->mNumAnimations && !node->mNumMeshes) {
            ...
            glm::mat4 locakMatrix = getGLMMat4(node->mTransformation);
            group->setLocalMatrix(locakMatrix);
        }
    ...
    void AssimpLoader::processSkeleton(
        ...
        // transform
        glm::mat4 boneMatrix = getGLMMat4(node->mTransformation);
        bone->mNodeMatrix = boneMatrix;
    ...
    Texture::Ptr AssimpLoader::processTexture(
        ...
        // 对于当前模型，工程里面存储的路径可能是:assets/models/superMan/man.fbx
        ...
        // 拼接后变成：assets/models/superMan/textures/diffuseTexture.jpg
        std::string fullPath = rootPath + aiPath.C_Str();
        return TextureLoader::load(fullPath);
    }


    // 1 如果当前解析的Mesh受到了骨骼的影响，那么就是一个SkinnedMesh
    // 2 如果受到了骨骼影响，那么其Attribute就需要加入skinIndex以及skinWeight
    Object3D::Ptr AssimpLoader::processMesh(
        ...
        std::vector<std::vector<float>> uvs;    // 每一个元素都是第i个channel的uv坐标数据
        ...
        for (uint32_t i = 0; i < mesh->mNumVertices; ++i) {
            ...
            for (uint32_t u = 0; u < mesh->GetNumUVChannels(); ++u) {
                ...
                // mNumUVComponents存储了当前第u个Channel所对应的这一套纹理坐标itemSize
        ...
        for (int b = 0; b < mesh->mNumBones; ++b) {
            ...
            // weightsNum代表了当前这根aiBone骨骼影响了多少个本mesh的顶点
    ...
    std::vector<AnimationClip::Ptr> AssimpLoader::processAnimation(const aiScene *scene) {
        ...
        for (uint32_t i = 0; i < animationNum; ++i) {
            ...
            for (uint32_t c = 0; c < animation->mNumChannels; ++c) {
                ...
                if (times.size()) {
                    // 很重要！！！这样写名字，可以用来在propertyBinding里面做正则表达式匹配
                    // 从而知道当前这个KeyframeTrack对应哪个骨骼的哪个属性
    ...
    void AssimpLoader::setVertexSkinData(
            const uint32_t &id, // 当前第几个vertex受到影响
            const float &boneId,    // 当前mesh受到影响的骨骼们构成一个数组bones，这个是里面的骨骼的id号
    ...
    // 系列函数工具，将向量、四元数、矩阵从assimp的格式里面转换为glm的格式
