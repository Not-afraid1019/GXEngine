...
namespace gx {
    // 对于每个mesh，将其所有顶点的某个Attribute共同存储成一个数组，比如Position就是一个float类型的数组
    ...
    private:
        ...
        // opengl或者其它图形api，将buffer分成了两种类型
        ...
        DataType            mDataType{DataType::FloatType}; // 本Attribute的数据类型

        bool mNeedsUpdate{true}; // 数据是否需要更新
        Range mUpdateRange{}; // 假设数组长度为300和float类型的数组，本次更新，可以只更新55-100个float数据
    };
    ...
    template<typename T>
    Attribute<T>::Attribute(const std::vector<T> &data, uint32_t itemSize, gx::BufferAllocType bufferAllocType) noexcept {
        mID = Identity::generateID();

        mData = data;
        mItemSize = itemSize;
        // 蕴含多少个顶点的信息
        mCount = static_cast<uint32_t>(mData.size());
...
//         发送消息，给到各类监听的函数，告诉它们哪个attribute已经消亡了
