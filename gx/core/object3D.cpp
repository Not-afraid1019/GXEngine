...
namespace gx {
    ...
    void Object3D::setQuaternion(float x, float y, float z, float w) noexcept {
        ...
        // 考虑到localMatrix可能已经被施加了scale方面的变换
    ...
    void Object3D::setScale(float x, float y, float z) noexcept {
        ...
        auto col1 = glm::normalize(glm::vec3(mLocalMatrix[0])) * y;
        auto col2 = glm::normalize(glm::vec3(mLocalMatrix[0])) * z;
    ...
    void Object3D::lookAt(const glm::vec3 &target, const glm::vec3 &up) noexcept {
        ...
        mLocalMatrix[3] = glm::vec4(position, 0.0f);
    ...
    glm::mat3 Object3D::updateNormalMatrix() noexcept {
        // normalMatrix由于存在scale的影响，不能直接变换，否则normal无法保证垂直与表面
    ...
    glm::mat4 Object3D::getNormalMatrix() noexcept {
        return mNormalMatrix;
    }
    ...
    void Object3D::addChild(const Object3D::Ptr &child) noexcept {
        ...
        // 给到child的是一个自身的智能指针。child是用weak_ptr来接纳的，避免了循环引用
        child->mParent = shared_from_this();

        // 查找当前节点在，是否已经加入了本child
