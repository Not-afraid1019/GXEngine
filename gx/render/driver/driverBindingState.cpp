...
namespace gx {
    ...
    // 寻找当前geometry是否曾经生成过一个DriverBindingState，如果没有，则新生成一个，否则把旧的返回
    ...
    void DriverBindingStates::setup(
        ...
        // 使用geometry寻找对应的DriverBindingState，如果有就给回，如果没有就连同vao重新生成一个
        ...
        // 这里处理了indexAttribute到DriverAttribute的对应，即EBO的创建
        // index应该与vao平级处理
        if (index != nullptr) {
            mAttributes->update(index, BufferType::IndexBuffer);
        }

        // 如果需要更新挂钩关系，那么就要在setVertexAttributes里面处理
        if (updateBufferLayout) {
    ...
    // 但是如果第一帧绘制完毕之后，第二帧继续绘制同样的VAO，那么这个挂钩关系的设置，就不需要做第二次
    // 本函数就用于检查当前的vao是否需要重新设置一次与各个vbo之间的挂钩关系
    // 需要重新挂钩的情况：
    // 1 geometry里面的attribute数量发生改变
    // 2 geometry里面的key所对应的attribute发生了变化，即调用了setAttribute，由于同样的key更换了新的attribute
    // 本Attribute会生成新的vbo，需要重新绑定
    ...
    bool DriverBindingStates::needsUpdate(const Geometry::Ptr &geometry, const Attributei::Ptr &index) noexcept {
        ...
        for (const auto& iter : geometryAttributes) {
            ...
            // 从缓存当中找到了这个attribute，那么就得对比id是否一致
            // 从而确定同样名字的attribute是否是同一个
    ...
    void DriverBindingStates::saveCache(const Geometry::Ptr &geometry, const Attributei::Ptr &index) noexcept {
        ...
        for (const auto& iter : attributes) {
            ...
            // 插入key-attributeNum，value-attributeID
    ...
    void DriverBindingStates::setupVertexAttributes(const Geometry::Ptr &geometry) noexcept {
        ...
        for (const auto& iter : geometryAttributes) {
            ...
            // itemSize:本attribute有多少个数字，比如position就是3个数字
            ...
            // 将attribute的location(binding)通过attribute的那么取出来
            ...
            //向vao里面记录，对于本binding点所对应的attribute，应该如何从bkAttribute->mHandle一个vbo里面读取数据
