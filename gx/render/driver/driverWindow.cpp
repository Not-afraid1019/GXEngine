...
namespace gx {
    ...
    DriverWindow::DriverWindow(gx::Renderer *renderer, const int &width, const int &height) noexcept {
        mWidth = width;
        mHeight = height;
        mRenderer = renderer;
        ...
        // 创建窗体
        mWindow = glfwCreateWindow(mWidth, mHeight, "GXEngine window", nullptr, nullptr);
        if (!mWindow) {
        ...
        // 将当前的DriverWindow这个类的对象的this指针作为一个userData传给了GLFWWindow(mWindow)
        // GLFWWindow Class里面含有一个void*的指针，作为用户可自定义的数据
        ...
        // 垂直同步的关闭，显示器有一定的刷新频率，大概率比渲染速度慢很多，开启
    ...
    bool DriverWindow::processEvent() noexcept {
        ...
        bool needsNotify{false};    // 键盘的某几个状态只要发生了改变，都会置为true，如果没有变化，不需要通知
        ...
        while (iter != KeyboardActionMap.end()) {
            ...
            {
                // 不为1，表示之前没有记录按下的状态
                if (!mKetState[iter->second]) {
                    // set函数将当前这个位置的bit设置为1
                    mKetState.set(iter->second);
                    needsNotify = true;
                }
            }
            else if (glfwGetKey(mWindow, iter->first) == GLFW_RELEASE) {
                // 如果当权者个按键是按下的状态，则更新其状态
                if (mKetState[iter->second]) {
                    // set函数将当前这个位置的bit设置为0
                    mKetState.reset(iter->second);
                    needsNotify = true;
                }
            }
            iter++;
        ...
        if (mKeyboardActionCallback && needsNotify) {
            mKeyboardActionCallback(mKetState);
        }
    ...
    void DriverWindow::setFrameSizeCallback(const gx::DriverWindow::FrameSizeCallback &callback) noexcept {
        mFrameSizeCallback = callback;
    }

    void DriverWindow::setMouseMoveCallback(const gx::DriverWindow::MouseMoveCallback &callback) noexcept {
        mMouseMoveCallback = callback;
    }

    void DriverWindow::setMouseActionCallback(const gx::DriverWindow::MouseActionCallback &callback) noexcept {
        mMouseActionCallback = callback;
    }

    void DriverWindow::setKeyboardActionCallback(const gx::DriverWindow::KeyboardActionCallback &callback) noexcept {
    ...
    void DriverWindow::frameSizeCallback(GLFWwindow *window, int width, int height) noexcept {
        ...
        // 将对应的DriverWindow指针从glfwWindow里面拿回来，拿回来的是一个void*
        // 非常之确定，这个void*指针一定是DriverWindow的指针
    ...
    void DriverWindow::mouseActionCallback(GLFWwindow *window, int button, int action, int mods) noexcept {
        // glfw给到哪个按键发生了反应(左中右)-button，发生了怎样的操作(点击、抬起)-action
