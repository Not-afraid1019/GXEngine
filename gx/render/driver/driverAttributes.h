...
namespace gx {
    ...
    // DriverAttribute是与Attribute一一对应
    ...
    public:
        ...
        // mHandle就是VBO
        // VertexBufferObject。代表了在GPU上的一块内存空间
        // 专门用来存储Mesh顶点的Attribute数据
        GLuint mHandle{0};
    };
    ...
    public:
        ...
        // 2 如果本Attribute拥有对应的DriverAttribute，则检查其VBO数据是否需要更新
        // bufferType表示了本Attribute是一个indexAttribute还是一个普通的Attribute
        template<typename T>
    ...
    DriverAttribute::Ptr DriverAttributes::update(
            const std::shared_ptr <Attribute<T>> &attribute,
          const gx::BufferType &bufferType
          ) noexcept {
        DriverAttribute::Ptr dattribute = nullptr;

        // 寻找map中是否有本Attribute对应的DriverAttribute
        auto iter = mAttributes.find(attribute->getID());
        if (iter != mAttributes.end()) {
            ...
            // bufferType要么是GL_ARRAY_BUFFER 要么是GL_ELEMENT_ARRAY_BUFFER
