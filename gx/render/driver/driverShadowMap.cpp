...
namespace gx {
    DriverShadowMap::DriverShadowMap(
            Renderer *renderer,
            const std::shared_ptr<DriverObjects> &objects,
    ...
    void DriverShadowMap::render(const std::shared_ptr<DriverRenderState> &renderState, const Scene::Ptr &scene, const Camera::Ptr &camera) noexcept {
        ...
        glm::vec2 shadowFrameExtents = glm::vec2(0.0); // 为了兼容点光源，但是在这里没有用
        ...
        for (uint32_t i = 0; i < lights.size(); ++i) {
            ...
            shadowFrameExtents = shadow->mFrameExtent;
            ...
            const Frustum::Ptr &frustum) noexcept
    {
        ...
        if (object->mIsRenderableObject) {
            ...
            if (renderableObject->mCastShadow) {
