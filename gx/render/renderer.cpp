...
namespace gx {
    ...
    bool Renderer::render(Scene::Ptr scene, Camera::Ptr camera) {
        ...
        // 经过上述projectObject的流程，任何一个我们使用到的Attribute都已经被成功的解析成为了一个VBO
    ...
    void Renderer::projectObject(const Object3D::Ptr &object, uint32_t groupOrder, bool sortObjects) noexcept {
        // 当前需要被解析的物体，如果是不可见物体，哪儿嘛连同其子节点一起都变为不可见状态
        if (!object->mVisible) return;

        glm::vec4 toolVue(1.0f);
        ...
        // 如果是可渲染物体
            ...
            if (mSortObject) {
                toolVue = glm::vec4(object->getPosition(), 1.0);
                toolVue = mCurrentViewMatrix * toolVue;
            }
    ...
    // 1 OpenGL是一个状态机
    // 2 每一帧都会调用多个DrawCall
    // 3 每一次DrawCall，OpenGL都会根据当前我们设置的各类状态进行绘制
    // 4 如果当前这一帧需要绘制三个物体，就得调用三次DrawCall
    // 5 绘制每一个物体之前，都必须对OpenGL的状态进行正确的设置（使用哪个VAO，使用哪一套Shader产生的Program，管线使用哪种）
    // 6 对OpenGL的API调用是有开销的，所以能复用就复用
    // 7 复用状态：假设三个物体都用了同样的Program，则不需要做三次Program的绑定，只需要调用一次UseProgram，绘制三个物体
    // 重要任务：
    // 拼装所有本次绘制需要的Uniforms到同一个outMap里面，然后进行统一的更新操作
    DriverProgram::Ptr Renderer::setProgram(
        ...
        // 如果本物体第一次送入到管线进行绘制，比如第一帧的第一个三角形，就必须为其生成一个DriverProgram
        ...
        if (material->mVersion == dMaterial->mVersion) {
            ...
            // 则Shader代码里面关于DiffuseMap的功能就不会被编译进去
            ...
            // 二者不相等会有两种情况，其一更换了Texture，这种变化不回更改Shader结构
        ...
        // 如果第一次解析material，测mCurrentProgram一定是nullptr
        ...
        // DriverMaterial根据我们绘制需要的material，对uniform进行了更新处理
        ...
        // 通用第一次会自动加入
        // uniforms是一个unordered_map，此时的uniform还没有加入如下的key-value组合
        // 此时如果按照如下方式访问，如果是空，那么就会自动创建一个条目（key-value）插入进去，只不过uniformHandle是默认值
...
//        DebugLog::getInstance()->end();
    ...
    DriverProgram::Ptr Renderer::getProgram(
        ...
        // 从用过的DriverPrograms里面找找看是否有这个类型的DriverProgram
        ...
        if (pIter != programs.end()) {
            ...
            // 根据Material的类型不同，取得其对应的特殊Uniform的map集合，即UniformMap
            dMaterial->mUniforms = mPrograms->getUniforms(material);
    ...
    // 统一了本Material跟其对应的DriverMaterial的关键变量，从而在下一帧的时候不会needsProgramChange
    ...
    void Renderer::makeLightsNeedUpdate(gx::UniformHandleMap &lightsUniformMap) noexcept {
