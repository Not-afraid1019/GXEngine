...
namespace gx {
    ...
    public:
        ...
        struct Listener {
            ...
            ~Listener() noexcept {
                if (mFunctionPointerDescriptor.mAddr) {
                    free(mFunctionPointerDescriptor.mAddr);
                }
            }
            ...
            template<typename T>
            static FunctionPointerDescriptor buildFunctionPointer(TypedFunctionPointer<T> functionPointer) {
                ...
                fpd.mAddr = p;  // p指向的内存中，存储了functionPointer的指针值
            ...
            FunctionPointerDescriptor mFunctionPointerDescriptor{};
        ...
        static bool listenerIsEqual(const Listener::Ptr& l0, const Listener::Ptr& l1) {
            ...
            return l0->mTarget == l1->mTarget && l0->mFunctionPointerDescriptor == l1->mFunctionPointerDescriptor;
        }
    ...
    protected:
        // 存储了监听事件名称-监听函数队列
        // 同一个事件名称可能会有多个Listener监听(多个对象的函数)
    ...
    void EventDispatcher::addEventListener(const std::string &name, T *target, TypedFunctionPointer<T> functionPointer) noexcept {
        // queueIter是当前这个消息(name)对应的map当中的键值对的迭代器,键值对是(string, ListenerQueue)
        // 对queueIter而言,first是消息名字,second是队列vector<Listener>
        ...
        // PS! listenerQueue是一个引用
        auto& listenQueue = queueIter->second;

        // functionPointer是一个类内的函数指针
        // target是对象的指针
        // bind操作将二者结合在一起，生成一个function
        ...
        // 通过bind生成的function无法进行对比，无法判断function1 == function2来判断是否相等
        // 传入的functionPointer是bind之前的结果，即单独的函数指针地址
        listener->mFunctionPointerDescriptor = Listener::buildFunctionPointer(functionPointer);
        listener->mFunction = function;

        auto listenIter = std::find_if(listenQueue.begin(), listenQueue.end(),
           [&](const Listener::Ptr& l) {
               return EventDispatcher::listenerIsEqual(listener, l);
        });
        if (listenIter != listenQueue.end()) {
            return;
        }

        listenQueue.push_back(listener);
    }
